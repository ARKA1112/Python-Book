...
class ListTree:
    """Mix-in that returns an __str__ trace of the entire class tree and all its objects attrs at and above self; run by print(), str() returns constructed string; uses __X attr names to avoid impacting clients; uses generator expr to recurse to superclasseswl uses str.format() to make substitutions clearer"""
    ...
    def __listclass(self, aClass, indent):
        ...
        if aClass in self.__visited:
            ...
            genabove = (self.__listclass(c, indent+4) for c in aClass.__bases__)    #generator expression
    ...
    def __attrnames(self, obj, indent):
        ...
        for attr in sorted(obj.__dict__):
            if attr.startswith('__') and attr.endswith('__'):
                result += spaces + '{0}=<>\n'.format(attr)  #format is a method
            else:
